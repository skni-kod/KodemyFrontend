...
export default function NavigationBar() {
  ...
  return (
    ...
    <div className={`h-full ${ExpandMenu ? '2sm:w-[14vw] sm:w-[8vw] lg:w-[6vw] xl:w-[5vw] 2xl:w-[4vw]' : '2sm:w-[47vw] sm:w-[23vw] lg:w-[20vw] xl:w-[16vw] 2xl:w-[14vw]'} flex flex-col ring-1 border-r-2 `} >
            ...
            <div className={`flex mt-auto mb-6 ml-auto mr-auto`}>
            <Image src={obj.lightMode} className='mr-3' alt='light-mode'/>
                <Switch
                    checked={DarkMode}
                    onChange={setDarkMode}
                    className={`${DarkMode ? 'bg-gray-800' : 'bg-gray-400'} relative inline-flex h-6 w-11 items-center rounded-full transition-all duration-500`}
                >
                    <span className={`${DarkMode ? 'translate-x-6' : 'translate-x-1'} inline-block h-4 w-4 transform rounded-full bg-white transition-all duration-500`}/>
                </Switch>
                <Image src={obj.nightMode} className='ml-3' alt='night-mode'/>
            </div>
    </div>
    </div>
  )
